...
<input type="text" name="" id="inp-op">
  ...
  {{-- --------------------------------- Form Add Objection ---------------------}}
  @if(!$objection and now()->diffInDays(Carbon::parse($order->delivery_date)) < 2 )
  <p>يمكنك الإحالة إلى التحكيم للاعتراض على الأعمال المنفذة خلال يومي عمل من تاريخ تنفيذها  </p>
  <button
    @click="show_form=!show_form" type="button" class="btn btn-danger w-25">
    إحالة إلى التحكيم
    </button>
    <form v-if='show_form' action="{{ route('client.objection.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      ...
      <div class="row ">
        ...
        <div class="col-12  mb-1">
          <label for="">
            <input type="checkbox" @input="sendContent" value="عدم تسليم الأعمال في المدة المحددة المتفق عليها" id=""
              v-model="reasons">
        ...
        <div class="col-12  mb-1">
          <label for="">
            <input type="checkbox" @input="sendContent" value="عدم الرد والاستجابة خلال المدة المحددة المتفق عليها"
              id="" v-model="reasons">
    ...
    <div class="tab-content" id="myTabContent">
          ...
          <div class="container">
            ...
            <div class="works_box flex-column">
              <div class="client d-flex gap-2 gap-sm-3">
                <div class="image_holder">
                  <a href="">
                    <h6 class="name">{{ $objection->user->username }}</h6>
                  </a>
                </div>
                <div class="info_text">
                  <p class="date mb-1">
              ...
              <div class="replays d-flex gap-3">
                <h6 class="mb-0">{{ $other->username }}</h6>
            ...
            <div class="vendor_decision">
              ...
              <div class="alert alert-danger">رد المحامي بالرفض
                <p>سبب الرفض: {{ $objection->venodr_refused_msg }}</p>
              </div>
              @else
            ...
            <form action="{{ route('client.objection.decision',$objection) }}" method="POST">
              @csrf @method('PUT')
              <input type="hidden" name="client_decision" value="accepted">
              <button type="submit" class="btn btn-success">قبول</button>
            </form>
            <button @click='refused=!refused' class="btn btn-danger">رفض</button>
            <form v-if="refused" action="{{ route('client.objection.decision',$objection) }}" method="POST">
              @csrf @method('PUT')
              <input type="hidden" name="client_decision" value="refused">
              <textarea name="client_refused_msg" @input="inpArbi($event)" class="form-control"></textarea>
      ...
      <div class="tab-pane fade" id="tap-3" role="tabpanel">
        ...
        <div id="printContract" dir="rtl" lang="ar">
          {!! $objection->judger_judgment !!}
        </div>
        @if($objection->client_objection)
        ...
        @if(now()->diffInDays($objection->judger_judgment_time) < setting('objection_duration')) <form
          action="{{ route('client.objection.judgment') }}" method="POST">
          @csrf
          <input type="hidden" name="objection_id" value="{{ $objection->id }}" id="">
          <textarea name="client_objection_reason" id="" cols="30" rows="10"></textarea>
