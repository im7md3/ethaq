    ...
    <section class="">
        ...
        <div class="section_content content_view">
            <div class="btn-holder d-flex align-items-center justify-content-center flex-wrap gap-2 mb-3">
                <a href="{{ route('admin.vendors.create') }}" class="main-btn btn-main-color">
                    إضافة
                    <i class="fa-solid fa-plus"></i>
                </a>
                <a href="{{ route('admin.vendors.index') }}" class="btn btn-primary">الكل: {{ $all }}</a>
                <a href="{{ route('admin.vendors.index', array_merge(request()->query(), ['status' => 'active'])) }}"
                    class="btn btn-green">محاميين مفعلين: {{ $active }}</a>
                <a href="{{ route('admin.vendors.index', array_merge(request()->query(), ['status' => 'not-active'])) }}"
                    class="btn btn-danger">محاميين غير
                    مفعلين: {{ $not_active }}</a>
            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-2">
                <div class="">
                    ...
                    <a href="{{ route('admin.vendors.index', ['license' => 'pending']) }}" class="btn btn-warning">بانتظار
                        التحقق: {{ $pendingLicense }}</a>
                    <a href="{{ route('admin.vendors.index', ['license' => 'active']) }}" class="btn btn-success">مفعلة:
                        {{ $activeLicense }}</a>
                    <a href="{{ route('admin.vendors.index', ['license' => 'no']) }}" class="btn btn-danger">بيانات غير مكتملة:
                        {{ $noLicense }}</a>
                    <a href="{{ route('admin.vendors.index', ['delete' => '1']) }}" class="btn btn-danger">حذف الحساب:
                        {{ $delete }}</a>
                </div>
                <div class="d-flex align-items-center flex-wrap gap-1">
                    <form action="{{ route('admin.vendors.index') }}" method="get" class="box-search">
                    ...
                    <a href="{{ route('admin.vendors.exports', request()->query()) }}" class="btn btn-info">تصدير<i
                        ...
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td class="d-flex gap-1">
                                    ...
                                    @can('update_users')
                                        <a href="{{ route('admin.vendors.edit', [$user->id]) }}" class="btn btn-info btn-sm">
                                            <i class="fa-solid fa-pen"></i></a>
                                    @endcan
                                    @can('read_users')
                                        <a href="{{ route('admin.vendors.show', $user->id) }}" class="btn btn-purple btn-sm">
                                    ...
                                    @can('delete_users')
                                        ...
                                        @include('admin.vendor.delete-modal', ['user' => $user])
                                    @endcan
                                </td>
                            </tr>
                        @endforeach
